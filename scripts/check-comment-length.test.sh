...
check GeneratedFile 0 '// Code generated by example. DO NOT EDIT.
// First line.
// Second line.
// Third line.'
...
printf '%s\n' '#!/bin/sh' "printf '%s\\000' 'file with space.go'" > "$tracked/bin/git"
chmod +x "$tracked/bin/git"
actual=0
(cd "$tracked" && PATH="$tracked/bin:$PATH" "$checker" >/dev/null 2>&1) || actual=$?
if [ "$actual" -ne 1 ]; then
